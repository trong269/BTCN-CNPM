...
def get_danh_sach_don_api(nhanvien_id: int):
    ...
    try:
        ...
        response.raise_for_status()
        data_list = response.json()  # API trả về danh sách đơn
        if not data_list:
            return pd.DataFrame(columns=['NgayTao', 'LyDo', 'TrangThai'])  # Trả về DataFrame rỗng
        # Chuyển đổi dữ liệu thành DataFrame
        for item in data_list:
            if 'ngaytao' in item and isinstance(item['ngaytao'], str):
                try:
                    item['ngaytao'] = date.fromisoformat(item['ngaytao'])
                except ValueError:
                    item['ngaytao'] = None  # Xử lý nếu định dạng ngày sai
